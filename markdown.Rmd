...
```{r}
hist(cpds, col="red", main="Steps Taken Each Day", xlab="steps", ylab="day")
```
...
dim(cpds)
```
#12. What is the impact of imputing missing data on the estimates of the total daily number of steps? Do these values differ from the estimates from the first part of the assignment?
```{r}
...
```
#13. Histogram of steps taken by day
```{r}
...
#10766.19
```
#15. Are there differences in activity patterns between weekdays and weekends?
```{r}
library(ggplot2)
library(plyr)
